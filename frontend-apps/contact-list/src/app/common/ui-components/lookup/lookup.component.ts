...
export class LookupComponent implements OnInit, OnChanges {
  ...
  resolveLabel(value: any): any {
    ...
    **/
    return this.lookupField && typeof value !== 'string' ? value[this.lookupField] : value;
  }

  constructor(private sfdcService: SfdcRemotingService) { }

  ngOnInit() {
    if (this.selection != null && this.selection[this.lookupField]) {
  ...
  ngOnChanges(changes: SimpleChanges) {
    if (changes.selection && !changes.selection.firstChange && changes.selection.currentValue
      ...
      && changes.selection.currentValue[this.lookupField]) {
